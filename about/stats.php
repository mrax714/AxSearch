...
echo number_format($sql->fetchColumn());
?>
   </strong>
   <h3>Last Crawled URLs</h3>
   <ul style="width: 400px;overflow: auto;">
   <?php
$sql = $dbh->query("SELECT `url` FROM `search` ORDER BY id DESC LIMIT 15");
while ($r = $sql->fetch())
{
    echo "<li style='margin-bottom:5px;'>" . "<a href='".$r['url']."' target='_blank'>".$r['url']."</a>" . "</li>";
}
?>
   </ul>
