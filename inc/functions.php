...
function head($title = "", $IncOtherCss = array())
{
    ...
    /* The Stylesheets */
    $cssFiles = array_merge(array(
        "all",
    ) , $IncOtherCss);
    foreach ($cssFiles as $css)
    {
        $url = preg_match("/http/", $css) ? $css : CDNHOST . "/css/$css.css";
        echo "<link href='" . $url . "?".rand()."' async='async' rel='stylesheet' />";
    }
...
function headerElem()
{ // header() is already a function in PHP
...
function getResults()
{
    ...
    if ($q != null)
    {
        ...
        $ends=array('','s','es','ing','ed','ies','ie','y');
        	$esqls[]="MATCH(title,url,description) AGAINST('+$q' IN BOOLEAN MODE)*3";
       
       foreach($ws as $w){
        foreach($ends as $end)
        {
        	$esqls[]="MATCH(title,url,description) AGAINST('$w$end' IN BOOLEAN MODE)";
        }
        }
        $csql="SELECT `image`,`title`, `url`, `description`, 
  ROUND((
  (".implode(' + ',$esqls).")/$wc)*100,1
  )
  AS `score` 
  FROM search 
  WHERE (".implode(' + ',$esqls).") > 0
  ORDER BY `score` DESC";
        $sql = $GLOBALS['dbh']->prepare($csql);
       // $sql->bindValue(":q", "$q");
         //  echo "\nPDO::errorInfo():\n";
